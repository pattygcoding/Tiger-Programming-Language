...
test("all reserved words, literals, and built-ins", () => {
  for (const text of "const def class super return if elif else while for in and or not".split(" ")) {
  ...
  for (const text of ["print", "str", "len"]) {
    assert.deepEqual(tokens(text), [{ text, kind: "builtin" }]);
  }
});

test("identifiers include Unicode and do not invent type keywords", () => {
  for (const text of ["number", "string", "bool", "int", "float", "self", "true_value", "printable", "caf\u00e9", "\u53d8\u91cf\u0661"]) {
